...
export class TsuguClient {
  ...
  constructor(opts: TsuguClientOptions = {}) {
    ...
    if (!addresses) {
      throw new Error(`tsugu: no deployment known for chain ${this.chainId}; pass { addresses }`);
    }
    this.addresses = {
    ...
    if (opts.privateKey) {
      if (!/^0x[0-9a-fA-F]{64}$/.test(opts.privateKey)) {
        throw new Error("tsugu: privateKey must be a 0x-prefixed 32-byte hex string");
      }
  ...
  private requireSigner(method: string): { account: Account; walletClient: WalletClient } {
    if (!this.walletClient || !this.account) {
      throw new Error(`tsugu: ${method} requires a privateKey`);
    }
  ...
  async hasEverOwned(owner: Address): Promise<boolean> {
    ...
    if (this.deployBlock > latest) {
      throw new Error(
        `tsugu: deployBlock ${this.deployBlock} is ahead of chain head ${latest} — refusing to report ownership (wrong network or misconfigured deployBlock?)`,
      );
    }
  ...
  async transferAgent(name: string, to: Address): Promise<Hash> {
    ...
    if (recipient === zeroAddress) throw new Error("tsugu: cannot transfer an agent to the zero address");

    const agent = await this.resolve(name);
    if (getAddress(agent.owner) !== account.address) {
      throw new Error(
        `tsugu: signer ${account.address} is not the owner of ${name}@tsugu (owner is ${agent.owner}); only the owner can transfer it`,
      );
    }
    ...
    const hash = await walletClient.writeContract(request);
    const receipt = await this.publicClient.waitForTransactionReceipt({ hash });
    if (receipt.status === "reverted") throw new Error(`tsugu: transfer of ${name}@tsugu reverted (tx ${hash})`);
    return hash;
  }
  ...
  async agentExecute(
    ...
    if (getAddress(owner) !== account.address) {
      throw new Error(
        `tsugu: signer ${account.address} is not the owner of agent wallet ${acct} (owner is ${owner}); execute is owner-gated`,
      );
    }
    ...
    const hash = await walletClient.writeContract(request);
    const receipt = await this.publicClient.waitForTransactionReceipt({ hash });
    if (receipt.status === "reverted") throw new Error(`tsugu: agent execute reverted (tx ${hash})`);
    return hash;
  }
  ...
  async sendWei(to: Address, value: bigint): Promise<Hash> {
    ...
    const recipient = getAddress(to);
    if (recipient === zeroAddress) throw new Error("tsugu: cannot send to the zero address");
