/**
 * Live verification of the tsugu coordination layer on Somnia Shannon:
...
const operator = new TsuguClient({ privateKey: PK, rpcUrls: [RPC] }); // poster + funder
const workerKey = generatePrivateKey();
const workerAddr = privateKeyToAccount(workerKey).address;
const worker = new TsuguClient({ privateKey: workerKey, rpcUrls: [RPC] });
...
async function main() {
  log(`\n■ tsugu coordination verification — capability "${CAP}" on Shannon (chain ${operator.chainId})`);
  ...
  log(`\n[0] worker agent ${NAME}@tsugu — token ${agent.tokenId}, wallet ${agent.account}  ${ex(agent.txHash)}`);
  ...
  log(`  worker=${NAME}@tsugu token=${agent.tokenId} wallet=${agent.account}`);
  log(`  advertise=${adTx}`);
